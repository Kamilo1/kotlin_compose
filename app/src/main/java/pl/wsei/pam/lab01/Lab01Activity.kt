...
class Lab01Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val progressLimit = 100
        val incrementValue = progressLimit / 6
        ...
        for (i in 1..6) {
            ...
            var mButton = Button(this).also{
                ...
                it.setOnClickListener{
                    Toast.makeText(this,"Testuj",Toast.LENGTH_LONG).show()
                    when (i) {
                        1 -> {
                            if (task11(4, 6) in 0.666665..0.666667 && task11(7, -6) in -1.1666667..-1.1666665) {
                                if(checkBox.isChecked == false){
                                    if (currentProgress + incrementValue <= progressLimit) {
                                        mProgress.progress += incrementValue
                                    } else {
                                        mProgress.progress = progressLimit
                                    }
                                }
    ...
    fun task16(store: Map<String, UInt>, asset: Map<String, UInt>): UInt {
        var minimalPossibleInstances: UInt? = null
        ...
        for ((assetItem, assetCount) in asset)
        {
            ...
            if (minimalPossibleInstances == null || possibleInstances < minimalPossibleInstances)
            {
                minimalPossibleInstances = possibleInstances
            }
        }

        return minimalPossibleInstances ?: 0U
    }
}